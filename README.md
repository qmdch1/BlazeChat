...
```powershell
.\scripts\benchmark.ps1 -Connections 1000 -Duration 30 -Warmup 10 -Drain 15 -MessagesPerSecond 2
...
```bash
CONNECTIONS=1000 DURATION=30 WARMUP=10 DRAIN=15 MESSAGES_PER_SECOND=2 ./scripts/benchmark.sh
```

연결 한계와 처리량을 직접 측정할 수도 있습니다.

```bash
docker compose run --rm bench \
  --url ws://proxy:8080/ws --mode connections \
  --connections 10000 --duration 30

docker compose run --rm bench \
  --url ws://proxy:8080/ws --mode throughput --connections 1000 \
  --messages-per-second 2 --warmup 10 --duration 30 --drain 15
```
